...
export const SettingsWorkspace = () => {
  ...
  // selects and menus portal outside this workspace and handle it themselves).
  useEffect(() => {
  ...
  const selectedPermissionOption =
    PERMISSION_MODE_OPTIONS.find(
      (option) => option.value === settings.defaultPermissionMode,
  ...
  return (
    ...
    >
      ...
      <header className="flex h-12 shrink-0 flex-row items-center pl-3 [-webkit-app-region:drag]">
        <div className="flex shrink-0 items-center gap-1">
          {isMacOs ? <div className="h-8 w-[5.25rem] shrink-0" /> : null}
          <Button
          ...
          <h1 className="font-semibold text-sm">{commonT("settings")}</h1>
        </div>
        <div className="h-full min-w-0 flex-1" />
        {!isMacOs && isElectron ? <WindowControls /> : null}
      </header>
