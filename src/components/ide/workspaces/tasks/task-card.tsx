...
const TaskCardImpl = ({
  ...
  // A finished task whose merge is still local can be pushed, after a
  // confirmation that says what else goes with it.
  const canPush =
    Boolean(task.completion) &&
    delivery?.branchExists === true &&
    delivery.pushed === false &&
    delivery.upstream !== null;
  ...
  return (
    <>
      ...
      >
        <div className="flex items-start gap-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent
              align="end"
              className="w-max min-w-28 max-w-64"
            >
              ...
              {canPush && delivery ? (
                <DropdownMenuItem onClick={() => setPushDialogOpen(true)}>
                  <ArrowUpFromLine className="size-4" />
                  {t("pushBranch", { branch: delivery.branch })}
                </DropdownMenuItem>
              ) : null}
      ...
      {/* Beside the card, not in it: the card's own click and double-click
        handlers would otherwise receive the dialog's events. */}
      {canPush && delivery ? (
        <PushDialog
          branch={delivery.branch}
          onCompleted={() => bumpProjectGitRefreshKey(project.id)}
