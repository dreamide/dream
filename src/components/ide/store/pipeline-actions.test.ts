...
import type { PipelineTask } from "@/types/ide";
...
import {
  ...
  finishPipelineRunInProjects,
  getCurrentPipelineRun,
  resolvePipelineStepAgent,
...
const createTestStore = () => {
  ...
  store.setState({
    ...
    ...createPipelineActions(store.setState, store.getState),
...
const getTasks = (store: TestStore): PipelineTask[] =>
  store.getState().projects[0]?.ui.pipelineTasks ?? [];

const getTask = (store: TestStore): PipelineTask => {
...
const addTask = (store: TestStore, projectId: string) => {
  const taskId = store.getState().addPipelineTask(projectId, {
...
test("adds, updates, and deletes pipeline tasks", () => {
  const { project, store } = createTestStore();

  assert.equal(
    store.getState().addPipelineTask(project.id, { title: "   " }),
    null,
  );

  const taskId = store.getState().addPipelineTask(project.id, {
    description: "  Details  ",
    title: "  Ship it  ",
  });
  assert.ok(taskId);
  ...
  store.getState().updatePipelineTask(project.id, taskId, { title: "Renamed" });
  ...
  store.getState().deletePipelineTask(project.id, taskId);
...
test("backlog tasks reorder without disturbing other steps", async () => {
  ...
  const ids = ["A", "B", "C"].map((title) => {
    const id = store.getState().addPipelineTask(project.id, { title });
    assert.ok(id);
    return id;
  });
  await store.getState().startPipelineTask(project.id, ids[1] as string);

  store.getState().movePipelineTaskInBacklog(project.id, ids[2] as string, 0);
  assert.deepEqual(
    getTasks(store)
  ...
  store.getState().movePipelineTaskInBacklog(project.id, ids[1] as string, 0);
  assert.equal(
...
test("starting a task runs the plan step in its own configured chat", async () => {
  ...
  const chatId = await store.getState().startPipelineTask(project.id, taskId);
  assert.ok(chatId);
  ...
  assert.match(
    worktreeRequests[0]?.branchName ?? "",
    /^pipeline\/task-[a-z0-9]+$/,
  );
  ...
  assert.equal(
    await store.getState().startPipelineTask(project.id, taskId),
    null,
  );
...
test("a finished plan waits for approval, then hands its output to build", async () => {
  ...
  const planChatId = await store
    .getState()
    .startPipelineTask(project.id, taskId);
  ...
  const buildChatId = await store
    .getState()
    .advancePipelineTask(project.id, taskId);
...
test("build auto-advances to review when the agent finishes normally", async () => {
  ...
  const reviewRun = getCurrentPipelineRun(task);
...
test("waiting, failed, and interrupted turns never advance a task", async () => {
  ...
  await flushMicrotasks();
  assert.equal(getTask(store).step, "build");
  assert.equal(getCurrentPipelineRun(getTask(store))?.finishedAt, null);

  // A busy step cannot be approved either.
  store.getState().setChatStreaming(buildChatId, true);
  assert.equal(
    await store.getState().advancePipelineTask(project.id, taskId),
    null,
  );
});
...
test("sending a task back reuses the step chat and carries the findings", async () => {
  ...
  assert.equal(
    await store.getState().sendPipelineTaskBack(project.id, taskId, "merge"),
    null,
  );

  const chatId = await store
    .getState()
    .sendPipelineTaskBack(project.id, taskId, "build", "Also add a test");
...
test("retrying a step starts a fresh chat with the same handoff", async () => {
  ...
  const retryChatId = await store
    .getState()
    .retryPipelineStep(project.id, taskId);
...
test("step chats run in the task's worktree project without stealing focus", async () => {
    ...
    projects: store.getState().projects.map((entry) =>
      entry.id === project.id
        ? {
            ...
            ui: {
              ...entry.ui,
              pipelineTasks: entry.ui.pipelineTasks.map((task) => ({
                ...task,
  ...
  assert.equal(state.activeProjectId, project.id);
  assert.equal(state.isPipelineChat(chatId), true);
  ...
  store.getState().openPipelineStepChat(project.id, taskId);
...
test("a failed worktree leaves the task in the backlog and reports why", async () => {
  ...
  await assert.rejects(
    store.getState().startPipelineTask(project.id, taskId),
...
test("a closed worktree project reopens in the background", async () => {
  ...
  assert.equal(
    await store.getState().retryPipelineStep(project.id, taskId),
    null,
  );

  assert.equal(
    await store.getState().reopenPipelineWorktree(project.id, taskId),
    true,
  );
  assert.ok(await store.getState().retryPipelineStep(project.id, taskId));

  // A worktree removed outside the pipeline cannot be reopened.
  store.setState({
    closedProjects: [],
    projects: store
      .getState()
...
test("completing a task records how it finished", async () => {
  ...
  store.getState().completePipelineTask(project.id, taskId, {
...
test("step config updates are per project and resettable", () => {
  const { project, store, worktree } = createTestStore();

  store.getState().setPipelineStepConfig(project.id, "plan", (config) => ({
    ...config,
    autoAdvance: true,
    prompt: "Custom {{task.title}}",
  }));
  const config = store.getState().projects[0]?.ui.pipelineConfig;
  assert.equal(config?.plan.autoAdvance, true);
  assert.equal(config?.plan.prompt, "Custom {{task.title}}");
  assert.equal(
    store.getState().projects.find((entry) => entry.id === worktree.id)?.ui
      .pipelineConfig.plan.autoAdvance,
    false,
  );

  store.getState().resetPipelineStepConfig(project.id, "plan");
  assert.equal(
    store.getState().projects[0]?.ui.pipelineConfig.plan.prompt,
    null,
  );
});

test("resolvePipelineStepAgent prefers the step model over defaults", () => {
  ...
  assert.deepEqual(
    resolvePipelineStepAgent({ model }, project, DEFAULT_SETTINGS),
    model,
  );

  const inherited = resolvePipelineStepAgent(
    { model: null },
    project,
    DEFAULT_SETTINGS,
  );
  ...
  assert.equal(
    resolvePipelineStepAgent({ model: null }, project, settings).model,
    getDefaultModelSelection(settings).model,
  );
});

test("finishPipelineRunInProjects returns the same reference when nothing changes", () => {
  const { project } = createTestStore();
  const projects = [project];
  assert.equal(
    finishPipelineRunInProjects(projects, "unknown", "", "now"),
    projects,
  );
});
