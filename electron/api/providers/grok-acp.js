...
export { AcpConnection as GrokAcpConnection } from "./acp-connection.js";

const getGrokPermissionMode = ({ permissionMode }) => {
  if (permissionMode === "full-access") return "bypassPermissions";
  if (permissionMode === "auto-accept-edits") return "acceptEdits";
  return "default";
};
...
const createGrokArgs = ({
  ...
  const args = [
    "--no-auto-update",
    "--permission-mode",
    getGrokPermissionMode({ permissionMode }),
    "agent",
    "--no-leader",
  ];
...
export const spawnGrokAcp = async (options = {}) => {
  ...
  return new AcpConnection(child, "Grok");
};
